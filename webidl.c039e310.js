...
var l = [
  ...
  "NaN",
  "null"
], y = a(f), d = ["callback", "dictionary", "enum", "interface"], p = a(d), v = ["typedef"], b = a(v), A = /^[:<=>?]/, g = /^-?([1-9][0-9]*|0[Xx][0-9A-Fa-f]+|0[0-7]*)/, h = /^-?(([0-9]+\.[0-9]*|[0-9]*\.[0-9]+)([Ee][+-]?[0-9]+)?|[0-9]+[Ee][+-]?[0-9]+)/, i = /^_?[A-Za-z][0-9A-Z_a-z-]*/, D = /^_?[A-Za-z][0-9A-Z_a-z-]*(?=\s*;)/, k = /^"[^"]*"/, E = /^\/\*.*?\*\//, C = /^\/\*.*/, w = /^.*?\*\//;
function N(e, r) {
  if (e.eatSpace())
    return null;
  if (r.inComment)
...
const S = {
  ...
  token: function(e, r) {
    ...
    if (t) {
      var n = e.current();
      r.lastToken = n, t === "keyword" ? (r.startDef = p.test(n), r.endDef = r.endDef || b.test(n)) : r.startDef = !1;
    }
    return t;
  },
  languageData: {

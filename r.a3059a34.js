...
var b = ["NULL", "NA", "Inf", "NaN", "NA_integer_", "NA_real_", "NA_complex_", "NA_character_", "TRUE", "FALSE"], g = ["list", "quote", "bquote", "eval", "return", "call", "parse", "deparse"], s = ["if", "else", "repeat", "while", "function", "for", "in", "next", "break"], y = ["if", "else", "repeat", "while", "function", "for"], h = f(b), N = f(g), m = f(s), A = f(y), k = /[+\-*\/^<>=!&|~$:]/, t;
function p(e, n) {
  t = null;
  var r = e.next();
  if (r == "#")
  ...
  if (/[a-zA-Z\.]/.test(r)) {
    e.eatWhile(/[\w\.]/);
    var i = e.current();
    return h.propertyIsEnumerable(i) ? "atom" : m.propertyIsEnumerable(i) ? (A.propertyIsEnumerable(i) && !e.match(/\s*if(\s+|$)/, !1) && (t = "block"), "keyword") : N.propertyIsEnumerable(i) ? "builtin" : "variable";
  } else
...
const I = {
  ...
  token: function(e, n) {
    if (e.sol() && ((n.ctx.flags & 3) == 0 && (n.ctx.flags |= u), n.ctx.flags & c && a(n), n.indent = e.indentation()), e.eatSpace())
      return null;

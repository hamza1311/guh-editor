function A(i) {
  ...
  ), w = /[\+\-\*\/!~&|^%=?:]/, k = /[\[\]{}()]/, b = /\d[0-9_]*/, I = /\d*\s*'s?d\s*\d[0-9_]*/i, O = /\d*\s*'s?b\s*[xz01][xz01_]*/i, T = /\d*\s*'s?o\s*[xz0-7][xz0-7_]*/i, W = /\d*\s*'s?h\s*[0-9a-fxz?][0-9a-fxz?_]*/i, B = /(\d[\d_]*(\.\d[\d_]*)?E-?[\d_]+)|(\d[\d_]*\.\d[\d_]*)/i, M = /^((\w+)|[)}\]])/, U = /[)}\]]/, d, v, K = h(
  ...
  function _(e, n) {
    ...
    if (e.eatWhile(/[\w\$_]/)) {
      var l = e.current();
      return g[l] ? (f[l] && (d = "newblock"), R[l] && (d = "newstatement"), v = l, "keyword") : "variable";
    }
    return e.next(), null;
  }
  function D(e) {
    return function(n, t) {
      for (var r = !1, l, m = !1; (l = n.next()) != null; ) {
        if (l == e && !r) {
          m = !0;
          break;
        }
      ...
      return (m || !(r || s)) && (t.tokenize = _), "string";
    };
  }
  function L(e, n) {
  ...
  return {
    ...
    indent: function(e, n, t) {
      ...
      var l = e.context, m = n && n.charAt(0);
      l.type == "statement" && m == "}" && (l = l.prev);
      var x = !1, $ = n.match(M);
      return $ && (x = N($[0], l.type)), l.type == "statement" ? l.indented + (m == "{" ? 0 : o || t.unit) : U.test(l.type) && l.align && !u ? l.column + (x ? 0 : 1) : l.type == ")" && !x ? l.indented + (o || t.unit) : l.indented + (x ? 0 : t.unit);
    },
    languageData: {
      indentOnInput: P(),
    ...
    token: function(i, o) {
      ...
      if (i.sol() && !o.tlvInBlockComment) {
        ...
        if (a <= o.tlvIndentationStyle.length) {
          var h = i.string.length == s, g = a * S;
          if (g < i.string.length) {
            var w = i.string.slice(g), k = w[0];
            E[k] && (c = w.match(q)) && z[c[1]] && (s += S, k == "\\" && g > 0 || (o.tlvIndentationStyle[a] = E[k], a++));
          }
          if (!h)
